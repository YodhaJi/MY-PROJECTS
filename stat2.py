...
def stat2(digits):  # stat1(): function for statment 1
    import random
    ...
    for j in index_list:
        ...
        while new_num_is_unique == True:
            ...
            if new_num not in digits and new_num not in s1:
                ...
                continue
    right_num_in_wrong_index = True
    while right_num_in_wrong_index == True:
        random.shuffle(s1)
        for i in index_list:
            if s1[i] in digits:
                right_num_in_wrong_index = False
    print(s1[0], '', s1[1], '', s1[2], ": One number is correct and wrong placed")

    #print(s1)  # one number is correct but wrongly placed
